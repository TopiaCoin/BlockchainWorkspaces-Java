...
public class UTPSocket extends Socket {
   ...
   private boolean closed = false;
	...
	public UTPSocket(InetAddress address, int port) throws IOException {
		...
		if (connectionIDReceive == Short.MAX_VALUE) {
			connectionIDReceive--;
			currentconnectionIDReceiveBytes[0] = intToFourBytes(connectionIDReceive)[2];
			currentconnectionIDReceiveBytes[1] = intToFourBytes(connectionIDReceive)[3];
		}		
    	currentconnectionIDSendBytes[0] = intToFourBytes(connectionIDReceive+1)[2];
		currentconnectionIDSendBytes[1] = intToFourBytes(connectionIDReceive+1)[3];
      ...
      byte [] next = new byte[1];
		...
		public int available() throws IOException {
				if (interupted == true) {
					...
					System.out.println(receivebuffer.size() + " bytes available on InputStream (closed == " + closed + ")");
  ...
  private byte [] intToFourBytes (int input) {
