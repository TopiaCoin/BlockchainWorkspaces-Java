			...
			while (runAgain) {
				...
				if (success) {
					if(firstTimeStartup) {
						...
						//Remove private keys from the wallet
						File tempWallet = new File(EOSConfigBaseDir + "\\wallet\\sdfstmp.wallet");
						if(tempWallet.exists()) {
							tempWallet.delete();
						} else {
							throw new RuntimeException("Failed to delete temp wallet at path " + tempWallet.getAbsolutePath());
						}
					}
